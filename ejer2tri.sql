...
declare	
	...
	begin
		if inserting then
			insert into auditarEmpl values(sysdate,:new.num_emp,:new.nombre,'INSERCION',null);
		end if;
		if deleting then
		...
		if updating then
			...
			insert into auditarEmpl values(sysdate,:old.num_emp,:old.nombre,'MODIFICACION',cambios);
		end if;		
	end;
/
